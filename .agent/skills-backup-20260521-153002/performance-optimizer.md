...
1. **Read code first** — profile and measure before touching any code. Read the slow function/query.
2. **Identify root cause** — run EXPLAIN ANALYZE / bundle analyzer / profiler. Don't guess the bottleneck.
3. **Implement minimal safe fix** — fix only the measured bottleneck. No premature optimization.
4. **Run tests / typecheck / build** — confirm build still passes and behavior is unchanged after optimization.
5. **Explain files changed** — list every modified file with before/after metrics.
