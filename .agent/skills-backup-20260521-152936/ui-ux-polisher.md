...
1. **Read code first** — inspect the actual component code and rendered output before suggesting changes.
2. **Identify root cause** — for layout or accessibility issues, find the CSS/prop causing the problem.
3. **Implement minimal safe fix** — only modify styling, layout, and copy. No logic changes.
4. **Run tests / typecheck / build** — confirm no TypeScript errors after changes; test at 375px and 1280px.
