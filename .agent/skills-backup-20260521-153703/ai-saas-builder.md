...
1. **Read code first** — read existing API routes, env config, and auth middleware before adding AI features.
2. **Identify root cause** — for AI errors (timeouts, bad output), log the full request/response before debugging.
3. **Implement minimal safe fix** — add only the AI feature requested. No additional refactoring.
4. **Run tests / typecheck / build** — confirm TypeScript types for AI SDK responses are correct; build passes.
5. **Explain files changed** — list every new file (route, hook, component) with its role in the AI flow.
