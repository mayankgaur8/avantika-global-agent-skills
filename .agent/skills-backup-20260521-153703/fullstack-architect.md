...
1. **Read code first** — read existing architecture files, schema, and API routes before designing.
2. **Identify root cause** — for messy codebases, identify the structural problem before prescribing a solution.
3. **Implement minimal safe fix** — recommend the architecture the project needs now, not future scale.
4. **Run tests / typecheck / build** — verify any schema or API changes don't break existing tests.
5. **Explain files changed** — for each new file or structural decision, explain its purpose.
