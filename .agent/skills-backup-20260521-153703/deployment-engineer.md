...
1. **Read code first** — inspect package.json/pom.xml/requirements.txt and existing config before writing deployment files.
2. **Identify root cause** — for deploy failures, read logs fully before guessing.
3. **Implement minimal safe fix** — add only what's needed for the target platform. No over-engineering.
4. **Run tests / typecheck / build** — production build must succeed locally before any deploy config is created.
5. **Explain files changed** — list every config file created/modified with its purpose.
