...
1. **Read code first** — run `git diff` and `git status` before staging anything.
2. **Identify root cause** — for failed commits (hooks, lint errors), read the error fully before fixing.
3. **Implement minimal safe fix** — stage only files that belong to this logical change.
4. **Run tests / typecheck / build** — confirm tests pass and build succeeds before committing.
5. **Explain files changed** — the commit message body should list what changed and why.
